...
#######
## change path of loco file in the list file: cat /medpop/esp/mesbah/GWAS_CHIP/MGBB53k/step1.NULL/NULL_MODEL_pred.list | sed 's:/broad/hptmp/mesbah/gwas/mgbb53k/:/medpop/esp/mesbah/GWAS_CHIP/MGBB53k/step1.NULL/:g' > /medpop/esp/mesbah/GWAS_CHIP/MGBB53k/step1.NULL/NULL_MODEL_pred.modified.list
## Male or Female only phenofile: 
# awk 'NR==1{print $0}(NR>1 && $5=="Male"){print $0}' /medpop/esp2/mesbah/projects/Meta_GWAS/rerun/mgbb53k/mgbb53k.imp_new.noRel_sk.21Jul2022.tsv > /medpop/esp2/mesbah/projects/Meta_GWAS/rerun/mgbb53k/mgbb53k.imp_new.noRel_sk.Male.Feb2024.tsv 

# awk 'NR==1{print $0}(NR>1 && $5=="Female"){print $0}' /medpop/esp2/mesbah/projects/Meta_GWAS/rerun/mgbb53k/mgbb53k.imp_new.noRel_sk.21Jul2022.tsv > /medpop/esp2/mesbah/projects/Meta_GWAS/rerun/mgbb53k/mgbb53k.imp_new.noRel_sk.Female.Feb2024.tsv

# multi-ancestry GWAS:
# to upper or to lower in bash: https://www.cyberciti.biz/faq/linux-unix-shell-programming-converting-lowercase-uppercase/
# cpus=5;sex='female';qsub -wd /broad/hptmp/mesbah/dataset/mgbb/gwas/tmpdir -t 1-$(wc -l /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/02.GWAS/1.regenie_GWAS/hg38_chrom_interval.tsv |awk '{print $1}') -R y -l h_vmem=5G -l h_rt=40:00:00 -pe smp ${cpus} -binding linear:${cpus} -N gwas.${sex}.mgbb53k /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/02.GWAS/1.regenie_GWAS/step02.regenie.stratified.sh /medpop/esp2/projects/MGB_Biobank/imputation/53K_GSA/release/bgen/GSA_53K.merged /medpop/esp2/projects/MGB_Biobank/imputation/53K_GSA/release/bgen/GSA_53K.merged /medpop/esp2/mesbah/projects/Meta_GWAS/rerun/mgbb53k/mgbb53k.imp_new.noRel_sk.${sex^}.Feb2024.tsv "hasCHIP,hasDNMT3A,hasTET2,hasASXL1,hasPPM1D,hasTP53,hasSF3B1,hasSRSF2,hasJAK2" "PC{1:10},Age_Genotyping,sqrAge_Genotyping" "Ancestry_Self_cat,Batch_CHIP_call" 400 /broad/hptmp/mesbah/dataset/mgbb/gwas/${sex} MGBB53k.${sex} 0.01 /medpop/esp/mesbah/GWAS_CHIP/MGBB53k/step1.NULL/NULL_MODEL_pred.modified.list 20 0.3 /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/02.GWAS/1.regenie_GWAS/hg38_chrom_interval.tsv ${cpus} ${sex}
...
OUT_PREFIX=${OUTPUT_PATH}/${COHORT}.chr${CHR_RANGE}.${Sex_Specific}
#########################################################################
######### Clock time

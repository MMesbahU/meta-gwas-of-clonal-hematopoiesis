...
##### 02 - files #####
phecodes_list <- fread("Data/mgb_phewas/phecode_list.tsv", data.table = F, header=F)$V1

# i=1
results_full <- data.frame()
for (i in seq_along(phecodes_list)){
    phecode <- phecodes_list[i]
    file_name <- glue("Results/mgbb_phewas/HR_mgbb_mesbahCHIP_Phewas_CoxPH.PheCode{phecode}.tsv")
...
write.table(results_full, "Results/mgbb_phewas/mgbb_phewas_results_n_incidents_count_full.tsv", 
...
results_full <- fread("Results/mgbb_phewas/mgbb_phewas_results_n_incidents_count_full.tsv", data.table = FALSE)
...
for (anc in ancestries) {
    ggsave(
        filename = glue("Results/mgbb_phewas/figures/phewas_plot_{anc}.png"),
        plot = plots[[anc]],

...
# qsub -R y -wd /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/tmpdir -N prep_sum -l h_rt=2:00:00 -l h_vmem=30G -pe smp 1 -binding linear:1 -t 1-$(wc -l /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/plots/list.gwama.maf001_p09_nstd2.txt | awk '{print $1}') /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/03.Meta-Analysis/03.Plots/plot_metaGWAS.python3.sh /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/plots/list.gwama.maf001_p09_nstd2.txt /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/out/plots /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/03.Meta-Analysis/03.Plots/Miami_Plot.n_qc.py
...
python ${py_manhattan} ${input_file} ${output_plot_mean_3sd} ${output_plot_median_3mad} ${output_plot_iqr} ${pheno_data}
#########

...
### 23 Jan, 2025
# mkdir -p /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/out/plots/perc
# 30% Sample size
# MAF>=0.1%
# qsub -R y -wd /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/tmpdir -N perc_plot_manhattan_eaf001 -l h_rt=2:00:00 -l h_vmem=30G -pe smp 1 -binding linear:1 -t 1-$(wc -l /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/plots/main_list.gwama.maf001_p09_nstd2.txt | awk '{print $1}') /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/03.Meta-Analysis/03.Plots/plot_metaGWAS.python3.eaf_n_perc.sh /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/input/plots/main_list.gwama.maf001_p09_nstd2.txt /broad/hptmp/mesbah/dataset/ch_gwas/GWAMA/out/plots/perc /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/03.Meta-Analysis/03.Plots/Miami_Plot.n_qc.eaf_n_perc.py 0.001 0.3
...
output_plot_perc=${outputDir}/perc${perc}.$(basename $(awk -v var=${SGE_TASK_ID} 'NR==var{print $1}' ${list_gwama_summary_file} ) ".tsv.gz").png
...
python ${py_manhattan} ${input_file} ${output_plot_perc} ${pheno_data} ${EAF} ${perc}
#########

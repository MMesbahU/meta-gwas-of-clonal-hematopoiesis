...
## Modified qqman::manhattan()
highlight_manhattan <- function (x, chr = "CHR", bp = "BP", p = "P", snp = "SNP", col = c("gray10", 
                                                                   "gray60"), chrlabs = NULL, suggestiveline = -log10(1e-05), 
          genomewideline = -log10(5e-08), highlight_new = NULL, col_new_highlight="red",
          highlight_old = NULL, col_old_highlight="green3", logp = TRUE, 
          annotatePval = NULL, annotateTop = TRUE, ...) 
{
  ...
  if (!is.null(highlight_new)) {
    if (any(!(highlight_new %in% d$SNP))) 
      warning("You're trying to highlight Novel SNPs that don't exist in your results.")
    d.highlight_new = d[which(d$SNP %in% highlight_new), ]
    with(d.highlight_new, points(pos, logp, col = col_new_highlight, 
                             pch = 20, ...))
  }
  # previously identified loci
  if (!is.null(highlight_old)) {
    if (any(!(highlight_old %in% d$SNP))) 
      warning("You're trying to highlight old SNPs that don't exist in your results.")
    d.highlight_old = d[which(d$SNP %in% highlight_old), ]
    with(d.highlight_old, points(pos, logp, col = col_old_highlight, 
                                 pch = 20, ...))
  }

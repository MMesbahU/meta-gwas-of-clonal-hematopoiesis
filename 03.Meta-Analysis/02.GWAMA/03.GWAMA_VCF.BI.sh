...
##### August 2023:
## while read trait; do qsub -R y -wd /broad/hptmp/mesbah/dataset/ch_gwas/tmpdir -N annot_gwama_has${trait}.Aug2023 -pe smp 1 -binding linear:1 -l h_rt=20:00:00 -l h_vmem=30G /medpop/esp2/mesbah/tools/CHIP_metaAnalysis/03.Meta-Analysis/02.GWAMA/03.GWAMA_VCF.sh /broad/hptmp/mesbah/dataset/ch_gwas/metaGWAS.has${trait}.topmed74k_topImpukb200k250k_mgbb53k_biovu54k.Aug2023.out.gz /broad/hptmp/mesbah/dataset/ch_gwas/tmp.metaGWAS.has${trait}.topmed74k_topImpukb200k250k_mgbb53k_biovu54k.Aug2023.vcf /broad/hptmp/mesbah/dataset/ch_gwas/sorted.metaGWAS.has${trait}.topmed74k_topImpukb200k250k_mgbb53k_biovu54k.Aug2023.vcf /broad/hptmp/mesbah/dataset/ch_gwas/sorted_metal.topmed74k_ukb200k250k_mgbb53k_biovu54k.multiAnc.${trait}.Aug2023.vcf.gz /broad/hptmp/mesbah/dataset/ch_gwas/metaGWAS.has${trait}.topmed74k_topImpukb200k250k_mgbb53k_biovu54k.Aug2023.vcf.gz 'P_METAL,N_METAL,EAF_METAL,BETA_METAL,SE_METAL' "/broad/hptmp/mesbah/dataset/RefSeq/dbSNP/hg38/chr1_X.hg38.GCF_000001405.39.vcf.gz" "/broad/hptmp/mesbah/dataset/ch_gwas/metaGWAS.has${trait}.topmed74k_topImpukb200k250k_mgbb53k_biovu54k.Aug2023.hg38_dbSNP.vcf.gz"; done < <(echo -e "CH\nDNMT3A\nTET2")
...
# zcat  /broad/hptmp/mesbah/gwas/ukb450k/metagwas/results/metaGWAS.TET2.TOPMed2019UKB200kUKB250kMGBB53kBioVU54k.out.gz | head -2
	# GWAMA Summary
zcat ${gwama_file} | awk 'NR>1 && $1~/^chr[0-9]+/ {print $0}' | sed 's:\::\t:g' | awk '{printf $1"\t"$2"\t"$1":"$2":"$3":"$4"\t"$3"\t"$4"\t.\t.\tvarID_hg38="$1":"$2":"$3":"$4";EffectAllele="$5";OtherAllele="$6";EAF="$7";BETA="$8";SE="$9";BETA_95L="$10";BETA_95U="$11";Z="$12";Pvalue="$13";log10_Pvalue="$14";Q_stat="$15";Q_Pvalue="$16";I_sqr="$17";N_Studies="$18";N_Samples="$19";Effect_Direction="$20"\n"}' >> ${tmpVCF}

...
# =============================================================================
gene_sub_dir <- "Results/sensitivity_analysis/onek1k_500genes_sub/"
n_batches    <- 12
batch_size   <- 5
...
gene_sub_results <- do.call(rbind, lapply(0:(n_batches - 1), function(i_batch) {
    ...
    score_files <- list.files(
        batch_dir,
        pattern = "\\.scdrs_group\\.predicted\\.celltype\\.l2$",
        full.names = FALSE
    )
    ...
    do.call(rbind, lapply(score_files, function(fname) {
        ...
        trait_full <- sub("\\.scdrs_group\\.predicted\\.celltype\\.l2$", "", fname)
...
for (trait in traits) {
    ...
    ggsave(
        paste0("Results/figures/onek1k_sensitivity/onek1k_gene_subsample_",
               trait, ".png"),
...
print(p_combined)
ggsave("Results/figures/onek1k_sensitivity/onek1k_gene_subsample_combined.png",
